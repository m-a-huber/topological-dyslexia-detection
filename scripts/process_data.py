...
sp_paths = sorted(Path("data/FixationReports").glob("*.txt"))
participants_stats_path = Path("data/DatasetStatistics/participant_stats.csv")

df_participants = pl.read_csv(participants_stats_path).filter(
    pl.col("subj") != "P14"  # drop because fixation data from P14 is missing
)
is_native = (
...
def process_fixation_reports(
    ...
    for sp_path in tqdm(
        ...
        out_file = time_series_dir / f"time_series_data_{id}.npy"
